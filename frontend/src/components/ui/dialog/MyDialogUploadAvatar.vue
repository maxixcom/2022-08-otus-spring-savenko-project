    ...
    <v-card>
      <v-form ref="form" v-model="form.formValidity" @submit.prevent="submit">
        ...
        <v-card-text class="my-0">
          <v-img v-if="!Boolean(avatarFile)" :src="userId | avatarUrl('small', hasImage, true) "></v-img>
          <vue-cropper
...
export default {
  ...
  methods: {
    ...mapActions('user', ['uploadAvatar', 'userAvatarDelete']),
    clean() {
    ...
    submit() {
      ...
      croppedCanvas.toBlob((croppedAvatar) => {
        this.uploadAvatar({ id: this.userId, file: croppedAvatar })
          .then(() => {
    ...
    remove() {
      this.busy = true
      this.userAvatarDelete({ userId: this.userId })
        .then(() => {
